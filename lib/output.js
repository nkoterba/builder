...
var ensureFilePath = require('./utils').ensureFilePath;
...
function createOutput(outputs, outFile, basePath, opts) {
  // process output
  var saucy = require('./sourcemaps');
  var sourceMap;
  ...
  if (opts.sourceMaps && outputObj.sourceMapsWithOffsets.length) {
    var sourceRoot = outFile ?
      path.dirname(path.resolve(basePath, outFile)) + path.sep :
      basePath;
...
exports.writeOutputs = function(opts, outputs, baseURL) {
  var basePath = ensureFilePath(baseURL);

  if (opts.outFile)
    opts.outFile = path.relative(basePath, path.resolve(opts.outFile));

  var output = createOutput(outputs, opts.outFile, basePath, opts);
